...
if grep -R -n -E '^import (AppKit|SwiftUI|UIKit)$|NotificationCenter|NSApplication|UIApplication' Packages/MiraioCore/Sources; then
...
if grep -R -n -E '(^|[^[:alnum:]_])Item([^[:alnum:]_]|$)|Add Item|Select an item' miraio miraioTests miraioUITests; then
    fail "starter Item behavior remains"
fi

application_products="$(grep -c 'productType = "com.apple.product-type.application"' miraio.xcodeproj/project.pbxproj)"
[[ "$application_products" == "1" ]] || fail "expected exactly one application product, found $application_products"

if grep -n -E 'iphoneos|iphonesimulator|xros|xrsimulator|TARGETED_DEVICE_FAMILY' miraio.xcodeproj/project.pbxproj; then
...
entitlement_count="$(plutil -p miraio/Miraio.entitlements | grep -c '=>')"
...
grep -q 'fetch-depth: 0' .github/workflows/ci.yml || fail "CI must fetch mainline history"
grep -q 'git rev-list --count origin/main' .github/workflows/ci.yml || fail "CI build numbers must use mainline commit count"
grep -q 'include-hidden-files:[[:space:]]*true' .github/workflows/ci.yml || fail "CI must include the hidden evidence directory in artifact uploads"
grep -q 'shell:[[:space:]]*bash' .github/workflows/ci.yml || fail "CI must propagate failures through evidence-capture pipelines"
